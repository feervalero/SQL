...
-- =============================================
IF EXISTS (SELECT * FROM MXBrands.sys.procedures where name = 'TempToAll') DROP PROCEDURE TempToAll;

GO
CREATE PROCEDURE TempToAll 
...
BEGIN
	...
	INSERT INTO [dbo].[AllProducts] (SKU,MaterialFeature,[Description],Price,BrandCode,Segmento,Category,Feature,FeatureDescription,FeatureType,[Version],StableVersion,UpdatedDateTime,CreatedDateTime)
		SELECT T.SKU,T.MaterialFeature,T.[Description],T.Price,T.BrandCode,T.Segmento,T.Category,T.Feature,T.FeatureDescription,T.FeatureType,@LastStableVersionOr+1,0,@time,@time
